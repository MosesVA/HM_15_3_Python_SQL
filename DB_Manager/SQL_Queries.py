def fill_employers(table_name, data_to_fill):
    """Функция для заполнения таблицы employers данными,
    принимает в качестве аргументов название таблицы и словарь.
...
def fill_vacancies(table_name, data_to_fill):
    """Функция для заполнения таблицы vacancies данными,
    принимает в качестве аргументов название таблицы и словарь.
    Возвращает SQL запрос"""
...
def fill_customers_data(table_name, data_to_fill):
    """Функция для заполнения таблицы customers_data данными,
    принимает в качестве аргументов название таблицы и словарь.
    Возвращает SQL запрос"""
...
def fill_employees_data(table_name, data_to_fill, employee_id):
    """Функция для заполнения таблицы employees_data данными,
    принимает в качестве аргументов название таблицы, словарь и employee_id(надо было для задания).
    Возвращает SQL запрос"""
    QUERY = fr"""INSERT INTO {table_name} (employee_id, first_name, last_name, title, birth_date, notes)
                            VALUES 
                            ('{employee_id}',
...
def fill_orders_data(table_name, data_to_fill):
    """Функция для заполнения таблицы orders_data данными,
    принимает в качестве аргументов название таблицы и словарь.
    Возвращает SQL запрос"""
